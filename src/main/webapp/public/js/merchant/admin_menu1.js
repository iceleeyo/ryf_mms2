 		...
 		$("#queryMenuAuthButtn").click(function (){//点击查询事件	
 			 MerchantService.searchOperAuth($("#mid").val(),$("#oper_id").val(),function(menuList){
 	 	 	 ...
 	 	 	 MerchantService.searchButtonAuth($("#mid").val(),$("#oper_id").val(),function(btBeanList){
 				var zTreeObj2 = $.fn.zTree.init($("#buttonAuth_tree"), setting, btBeanList);
 	 	 	 });
...
// 		            nameIsHTML: true  
 		...
 		function getFont(treeId, node){
	        return node.isChanged ? {color:"red"} : {};  
 		}
 		//审核 权限申请 提交
 		$(".btn").live('click',function(){
 			...
 			MerchantService.doCheckAuthApply(mid,operId,status,function(msg){
 				alert(msg);
 				showOperApply(1);
 			});
 		});
