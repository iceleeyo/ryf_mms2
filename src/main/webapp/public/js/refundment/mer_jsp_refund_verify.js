    ...
    function submitCheckAccount(action) {
        ...
        if (action == "retroversion"){
            if (confirm("撤销退款?")) {  
                ...
                MerRefundmentService.verifyFail(arr,refundIds,callback);
            }
        
       } else {
           if (confirm("确定退款?")) {  
               MerRefundmentService.verifySure(refundIds,callback);
           }
       }
    }
    ...
    function queryVerifyRefund(pageNo){
        ...
        var dateState = 1;
        MerRefundmentService.queryRefundLogs( pageNo,mid,5,tseq,dateState,bdate,edate,gate,orgid,null,null,callBack2);
    }
