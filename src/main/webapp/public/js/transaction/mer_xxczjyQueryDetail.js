...
//            dwr.util.addOptions("bkCheck", h_bk_chk);
        ...
        //查询条件的判断
        var mid,gate,tstat,date,bdate,edate,gateRouteId,bkCheck,tseq,oid,bkseq,type,batchNo,rg_tstat;
        ...
        function querypaymentHlog(pageNo){
            ...
            TransactionService.querypaymentHlogDetail_cz_mer( pageNo,tstat,date,bdate,edate,tseq,oid,null,rg_tstat,callBack2);
        }
        //明细查询下载
        function downloadpaymentDetail(){
        	if(!judgeCondition())return;
            dwr.engine.setAsync(false);//把ajax调用设置为同步
            TransactionService.downloadpaymentDetail_cz_mer(tstat,date,bdate,edate,tseq,oid,null,rg_tstat,function(data){dwr.engine.openInDownload(data);});
        }
