      ...
      function initDate(){
        ...
        function confirmPhonePay(){
            ...
            var userOid=$("userOid").value.trim();
            ...
            var uid = $("uid").value;
            var mid = $("mid").value;
            MerTransactionService.doPhonePay( mid, transAmt, phoneNo, payTimePeriod, uid,userOid,function(msg){
        	   if(msg == 'ok'){
                   alert("发送订单支付短信成功!");
                  // $("ordId").value='';
                   $("transAmt").value='';
        ...
        //手机支付查询
		 function queryPhonePay(pageNo){
	            ...
	            MerTransactionService.queryPhonePay( pageNo, table, mid, tstat, bdate, edate, operid,callBack2);
	     }
		 //手机支付下载
	    function downloadMobilePay(){
	        	if(!judgeCondition())return;
	            dwr.engine.setAsync(false);//把ajax调用设置为同步
	            MerTransactionService.downloadPhonePay(table, mid, tstat, bdate, edate, operid,
...
/*
	        function sendMerPayUrl(tseq,mid,oid,transPeriod,mobileNo){
	        	MerTransactionService.reSendPayUrl(tseq,mid,oid,transPeriod,mobileNo,function(msg){
	                if(msg.indexOf('ok')!=-1){
	                    alert("发送成功!");
