...
(function($){
    ...
    $.fn.wBox = function(options){
        ...
        var  wBoxHtml = '<div id="wBox"><div class="wBox_popup"><table><tbody><tr><td class="wBox_tl"/><td class="wBox_b"/><td class="wBox_tr"/></tr><tr><td class="wBox_b"><div style="width:10px;">&nbsp;</div></td><td><div class="wBox_body">' 
        				+ (_this.YQ.noTitle ? '' : '<table class="wBox_title"><tr><td class="wBox_dragTitle"><div class="wBox_itemTitle">' + _this.YQ.title + '</div></td><td width="20px" title="关闭"><div class="wBox_close" id="wBox_close"></div></td></tr></table> ') +
        ...
        this.showBox=function (){
            ...
            $("#wBox_overlay").remove();
			$("#wBox").remove();
			var bgHeight=getPageHeight();
			var iframStr='<iframe style="border:0px none;filter:alpha(opacity=0);background-color:gray;width:100%;height:'+bgHeight+'px;"/>';
           B = $("<div id='wBox_overlay' class='wBox_hide'>"+iframStr+"</div>").hide().addClass('wBox_overlayBG').css({'height':bgHeight,'opacity':_this.YQ.opacity}).appendTo('body').fadeIn(100);
          //  B = $("<div id='wBox_overlay' class='wBox_hide'>"+iframStr+"</div>").hide().addClass('wBox_overlayBG').css('opacity',_this.YQ.opacity).appendTo('body').fadeIn(100);
        ...
        function handleClick(){
            ...
            afterHandleClick();
           // $("select").not($("#wBoxContent select")).hide();//为了兼容IE6的div不能覆盖select
            $t.find("*[id]").map(function(){//id add '_'
        ...
        this.close=function (){
            if (C) {
            	//$("select").show();
                $t.find("*[id]").map(function(){//id remove '_'
                   var idStr=$(this).attr("id");
               	   $(this).attr("id",idStr.substr(1,idStr.length));
               });
                B.remove();
                C.stop().fadeOut(100, function(){
                    C.remove();
                })
            }
        }
