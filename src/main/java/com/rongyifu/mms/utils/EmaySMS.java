...
public class EmaySMS {
	...
	private final static String SOFTWARESERIALNO = "9SDK-EMY-0999-JCVML";
	public final static String password="953105";// 密码,请通过亿美销售人员获取
	
	static{
		if (client == null) {
			try {
				client = new Client(SOFTWARESERIALNO, password);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static int sendSMS(String[] phone, String content) {
		try {
			if(phone.length==0||"".equals(phone[0].trim())){
				LogUtil.printErrorLog("EmaySMS", "sendSMS", "发送失败,手机号为空!");
				return 1;
			}
			/**判断内容是否包含【电银信息】			 * */
			/***短信内容请在头部加【电银信息】，否则的话 就别加***/
			if(!content.contains("【电银信息】")){
				content="【电银信息】"+content;
			}
			content=content+"回复TD退订";
			String phones=StringUtils.join(phone, ",");
			LogUtil.printInfoLog("EmaySMS", "Send", "phones:"+phones+" content:"+content);
			return client.sendSMS(phone, content, "", 5);// 带扩展码
		} catch (Exception e) {
			LogUtil.printErrorLog("EmaySMS", "sendSMS", e.getMessage());
			return 1;
		}
	}
...
//		try {
//			String a = SingletonClient.getClient().getBalance();
