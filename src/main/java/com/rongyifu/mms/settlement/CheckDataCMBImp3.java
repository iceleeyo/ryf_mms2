 ...
 *
 */
public class CheckDataCMBImp3 implements SettltData {
	// 招商银行txt文件对账
	@Override
...
//		 清算系统交易流水号         清算系统交易日期     定单号            交易类型    交易金额     回扣    入账日期 
//		12282973300000000240 20120829        0001627657    结账                  630  3.15  20120829
//		12282973300000000241 20120829        0001627702    结账                1330  6.65  20120829
		...
		for (String line : datas) {
			...
			if (val != null && val.length != 7 && lineCount < datas.length) {
				return res;
			}
			SBean bean = new SBean();
			bean.setGate(bank);
			bean.setTseq(Long.parseLong(val[2]) + ""); // 订单号
			bean.setDate(val[6]); // 入账日期
			bean.setAmt(val[4].replaceAll(",", "")); // 交易金额
			bean.setBkFee(val[5].replaceAll(",", "")); // 回扣（银行手续费）
			res.add(bean);
