 ...
 *
 */
public class CheckDataCMBImp implements SettltData {
	// 招商银行txt文件对账
	@Override
	public List<SBean> getCheckData(String bank,String fileContent) throws Exception {
		...
		// 20110603,20110604,1160.00,0000069151,0,11260430100000000050
		int lineCount = 0;
		for (String line : datas) {
			// System.out.println(line);

			String[] val = line.split(",");
			if (val != null && val.length != 6 && lineCount < datas.length) {
				return res;
			}
			SBean bean = new SBean();
			bean.setGate(bank);
			bean.setTseq(Long.parseLong(val[3]) + "");
			// bean.setBkMdate(val[0]);
			bean.setDate(val[1]);
			bean.setAmt(val[2].replaceAll(",", ""));
			// bean.setBkFee(null);
			res.add(bean);
