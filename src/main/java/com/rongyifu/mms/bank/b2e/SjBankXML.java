 ...
 *
 */
public class SjBankXML implements BankXML {
	...
	public String genSubmitXML(int trCode, B2EGate gate) throws B2EException {
		...
		if(trCode==B2ETrCode.QUERY_ACC_BALANCE){
			...
			xml.append("<cur_code>").append(SjBankXML.cur_code).append("</cur_code>");
			xml.append("</body>");
			xml.append("</a>");
	...
	public String genSubmitXML(int trCode, TrOrders os, B2EGate gate)
		...
		if(trCode==B2ETrCode.PAY_TO_OTHER){ //对外支付
			...
			xml.append(getHead(os, gate, "300001"));//对外支付接口编码 （300001）
			xml.append("<body>");
			xml.append(getXml_300001(os, gate));
			xml.append("</body>");
			xml.append("</a>");
			return addHead(xml.toString());
	...
	public void parseXML(B2ERet ret, String xml) throws B2EException {
		...
		/*	if(body==null){
			...
			if("200108".equals(trCode)){ //余额查询接口
				if(SjBankXML.suc_ret.equals(retCode)){
			...
			LogUtil.printErrorLog("SjBankXML", "parseXML", xml, e);
		}
	...
	private String getXml_529301(TrOrders os,B2EGate gate){
		...
		xml.append("<pay_cur_code>").append(SjBankXML.cur_code).append("</pay_cur_code>");
		...
		xml.append("<purpose>").append(os.getRemark()).append("</purpose>");
		xml.append("<postscript>").append(os.getRemark()).append("</postscript>");
	...
	private String getXml_300001(TrOrders os,B2EGate gate){
		...
		String bg= !Ryt.empty(bk_no) && bk_no.length() > 3 ? bk_no.substring(0, 3) : bk_no;
		String bankFlag="1";
		if(bg.equals("313")){
		...
		xml.append("<pay_acno>").append(gate.getAccNo()).append("</pay_acno>");
		xml.append("<pay_cur_code>").append(SjBankXML.cur_code).append("</pay_cur_code>");//币种
		xml.append("<pay_acname>").append(gate.getAccName()).append("</pay_acname>");
		...
		xml.append("<rcv_cur_code>").append(SjBankXML.cur_code).append("</rcv_cur_code>");//币种
	...
	private void parseXml_300001(B2ERet ret,Element head,Element body){
		String succFlag=head.elementText("succ_flag");
		String respMsg=head.elementText("ret_info");
		String serialNo=head.elementText("serial_no");
		if("0".equals(succFlag)){//交易成功
			String tr_date=head.elementText("tr_acdt");
			String tr_time=head.elementText("tr_time");
			ret.setBank_date(tr_date);
			ret.setBank_time(tr_time);
			ret.setTransStatus(0);// 成功
			ret.setSucc(true);
			ret.setMsg(respMsg);
			ret.setErrorMsg(respMsg);
			ret.setResult(serialNo);
	...
	private void parseXml_200205(B2ERet ret, String retCode, String respMsg,
		...
		if(SjBankXML.suc_ret.equals(retCode)){
			...
			String error_info=head.elementText("ret_info");
