...
public class BankcommBEXML implements BankXML {
	
	Logger log4j = Logger.getLogger(BankcommBEXML.class);
...
//		}
				...
				String bg=(o.getToBkNo()!=null && o.getToBkNo().length()>3)?o.getToBkNo().substring(0, 3):o.getToBkNo();
				if (bg.equals("301")&&o.getPtype()==7){// 付款到个人银行卡
					tr_code = "330002";
				} else {
					// 去掉code=8 B2ETrCode=8 账户信息查询
					tr_code = "210201";
				}
		}
	...
	public String genSubmitXML(int code, B2EGate gate) throws B2EException {
		...
		if (code == B2ETrCode.QUERY_ACC_BALANCE) {
			...
			//当日交易明细查询
			genSubHead(resXml, "310201", corp_no, user_no, req_no, "1");
			resXml.append("<body><acno>").append(gate.getAccNo()).append("</acno></body>>");
		}else if(code == B2ETrCode.QUERY_HISTORICAL_TRADE){
			//历史交易明细查询
			genSubHead(resXml, "310301", corp_no, user_no, req_no, "1");
			resXml.append("<body><acno>").append(gate.getAccNo()).append("</acno>");
			resXml.append("<start_date>").append("20130321").append("</start_date>");
			resXml.append("<end_date>").append("20130321").append("</end_date></body>");
...
//			res.append("<rcv_exg_code>").append("").append("</rcv_exg_code>");
			...
			int bank_flag=StringUtils.left(o.getToBkNo(), 3).equals("301") ? 0 : 1;
...
//		}
	...
	public static void main(String args[]){
		BankcommBEXML bank  = new BankcommBEXML();
	...
	public void parseXML(B2ERet result,String xmlString) throws B2EException {
		...
		try {
			...
			if (body == null && !tr_code.equals("210201")){
				LogUtil.printErrorLog("BankcommBEXML", "parseXML", "返回报文没有Body");
			...
			if(tr_code.equals("210201")){
				if(result.isSucc()){
					LogUtil.printInfoLog("BankcommBEXML", "parseXML", "交易码210201 交易成功");
					result.setTransStatus(2);//设置成处理中 
					return;
				}else{
					LogUtil.printInfoLog("BankcommBEXML", "parseXML", "交易码210201 交易失败");
					return;
				}
			}
			// 查询余额
...
//					result.setErr("状态未知");
			...
			if (tr_code.equals("330002")) {
				...
				LogUtil.printInfoLog("BankcommBEXML", "parseXML", "sucFlag:"+sucFlg+"<<<<");
