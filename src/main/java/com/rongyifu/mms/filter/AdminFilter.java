...
public class AdminFilter implements Filter {
	...
	private static final String ADMINT = "A";

	@Override
	public void init(FilterConfig filterConfig) throws ServletException {
		String contextPath = filterConfig.getServletContext().getContextPath();
		redirectURL = contextPath+"/admin/login.jsp";
	}
	...
	public void doFilter(ServletRequest sRequest, ServletResponse sResponse, FilterChain filterChain)
		...
		if (session == null || lu == null) {
			...
			try {
				...
				if (validateUrl(lu, reqUrl)||reqUrl.indexOf(".html") != -1) {
					filterChain.doFilter(sRequest, sResponse);
				} else {
				...
				LogUtil.printErrorLog(getClass().getCanonicalName(), "doFilter", "", e);
				response.getWriter().print(HTMLPage.noAuthHtmlPage());
			}
		}
	}
	...
	public boolean validateUrl(LoginUser user, String reqUrl) {
		...
		if (reqUrl.indexOf("mer") != -1) return false;
		...
		String t = r[0];
		String id = r[1];
		if (!t.equals(ADMINT)) return false;
